...
function Create-AzureVM
{
    ...
    (
        ...
        [Parameter(Mandatory=$false, HelpMessage='Specify the resource group, if not specified the servicename will be used')]
     ...
     :outer while ($true)
       {
        If($EnableExentions)
        {
        New-OperationalInsightsWorkspace -StorageName $StorageName -Location $Location -WorkspaceName $OperationalInsightsName -ServiceName $ServiceName
        ...
        if(!($ResourceGroup)){$ResourceGroup = $ServiceName}
